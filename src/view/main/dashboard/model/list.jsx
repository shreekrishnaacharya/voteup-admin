import { Typography, Box, Pagination, Skeleton, Badge } from '@mui/material';
...
const getPostsPie = (dataL) => {
  let labels = [],
    data = [];
  const mapper = {};
  Object.keys(StatusList).forEach((e, i) => {
    mapper[e] = i;
    labels[i] = StatusList[e].name;
    data[i] = 0;
  });
  dataL.forEach((da) => {
    labels[mapper[da.status]] = da.name;
    data[mapper[da.status]] = da.value;
  });
  return {
    labels,
    datasets: {
...
function Text({ text, edge, warpLength }) {
  return (
    <Box
      ...
      pb={1.25}
    >
      <Typography warpLength={warpLength}>{text}</Typography>
    </Box>
  );
}
...
function Status({ status }) {
  return (
    <Badge
      variant="gradient"
      badgeContent={StatusList[status].name}
